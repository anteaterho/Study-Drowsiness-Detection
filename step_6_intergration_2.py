...
EAR_THRESHOLD = 0.25
# 눈 감김 지속 프레임 수 임계값 (예: 48프레임 = 약 2초)
...
frame_counter = 0
solenoid_tapping = False  # 솔레노이드 탭 동작 중인지 추적
    ...
    if results.multi_face_landmarks:
        for face_landmarks in results.multi_face_landmarks:
            ...
            if avg_EAR < EAR_THRESHOLD:
                ...
                state = "CLOSED"
                color = (0, 0, 255) # 빨간색
                # 졸음 판정 프레임 초과 시 솔레노이드 탭 시작 (중복 전송 방지)
                if frame_counter >= CONSECUTIVE_FRAMES and not solenoid_tapping:
                    if arduino:
                        arduino.write(b'T')  # Tapping 시작
                ...
                if solenoid_tapping:
                    if arduino:
                        arduino.write(b'S')  # Stop tapping
